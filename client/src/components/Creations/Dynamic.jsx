...
const Dynamic = () => {
    ...
    const { data:specificData,error } = useSpecificPortfolioQuery(SelectValue.replace(/\s+/g, "-").toLowerCase().trim());
            ...
            <div className={css.grid_conatiner}>

                {
                    data?.Portfolio.map(({ poster, video, name, _id }, index) => {
                        console.log(poster)
                        return (
                            <div key={_id} className={css.item} style={{ backgroundImage: `url(${poster.url})` }}>
